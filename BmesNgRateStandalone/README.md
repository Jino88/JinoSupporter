...
Every file under `Services\BmesReports` is generated by that script; edit the `JinoSupporter.Web` original instead, because the next build overwrites the copy. The standalone syncs only the F-Cost calculation slice of `BmesReports` — the calculation service `BmesFCostPage` injects, the models it returns, the projection helper, and the DTO contracts. The daily, weekly, cause-monthly and KPI calculation services, the orchestrator and the report viewer bootstrap stay in the web app, which is the only side with a report host. The script's file list is the single place that boundary is declared, and it fails the build rather than sync silently if a source it expects has moved.
