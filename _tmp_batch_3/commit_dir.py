"""Commit every .json file in _tmp_batch_3/payloads/ that hasn't been marked done."""
...
PAY_DIR = os.path.join(ROOT, '_tmp_batch_3', 'payloads')
DONE_DIR = os.path.join(ROOT, '_tmp_batch_3', 'done')
...
ok = 0
fail = 0
for fp in sorted(glob.glob(os.path.join(PAY_DIR, '*.json'))):
    try:
        ...
        os.rename(fp, os.path.join(DONE_DIR, base))
        ok += 1
        print(f"[OK] {name}")
