...
public static class AppRoles
{
    ...
    public const string ManagerAi = "Manager(AI Possible)";
    public const string Leader    = "Leader";
    public const string Editor    = "Editor";
    public const string Viewer    = "Viewer";

    public static readonly string[] All = [Admin, Manager, ManagerAi, Leader, Editor, Viewer];

    // Can input data & use AI extract/tags
    public static bool CanEdit(System.Security.Claims.ClaimsPrincipal user)
        => user.IsInRole(Admin) || user.IsInRole(Manager) || user.IsInRole(ManagerAi)
           || user.IsInRole(Leader) || user.IsInRole(Editor);
